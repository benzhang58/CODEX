...
class EmailSummarizer:
    ...
    def generate_summary(self, records: List[EmailRecord], contact_name: Optional[str] = None, is_overall: bool = False) -> str:
        ...
        section_guide = (
            "Output ONLY clean Markdown using exactly these sections in order:\n\n"
            "## Thread Context\n"
            "What was the conversation history leading up to the new email(s)? "
            "Who said what, and when? Omit this section entirely if every email is standalone.\n\n"
            "## Executive Summary\n"
            "What is this person communicating in their latest message(s)?\n\n"
            "## Main Topics\n"
            "Key subjects discussed across the thread(s).\n\n"
            "## New Developments\n"
            "What is new or changed compared to prior messages in the thread.\n\n"
            "## Action Items / Asks\n"
            "Anything requested of you or others, explicit or implicit.\n\n"
            "## Deadlines / Dates / Meetings\n"
            "All time-sensitive items mentioned anywhere in the thread.\n\n"
            "## Risks / Things to Watch\n"
            "Issues, blockers, or concerns raised anywhere in the thread.\n\n"
            "## Attachment Summary\n"
            "For EVERY attachment across the ENTIRE thread (from ANY sender): "
            "state the filename, who sent it, when, what it contains, and why it matters. "
            "Do not skip any attachment. If there are none, write 'None.'\n\n"
            "## Bottom Line\n"
    ...
    def _markdown_to_html(self, text: str) -> str:
        ...
        suitable for an email body. Handles # headings, - bullets, **bold**.
        """
        import re
        ...
        for line in lines:
            ...
            # Close open list before a non-bullet line
            ...
            # Headings → bold + underline, with clear spacing
            if s.startswith("### "):
                t = self._inline_html(s[4:])
                html.append(f'<h3 style="margin:16px 0 4px; text-decoration:underline;">{t}</h3>')
            elif s.startswith("## "):
                t = self._inline_html(s[3:])
                html.append(f'<h2 style="margin:20px 0 6px; text-decoration:underline;">{t}</h2>')
            elif s.startswith("# "):
                t = self._inline_html(s[2:])
                html.append(f'<h1 style="margin:24px 0 8px; text-decoration:underline;">{t}</h1>')
            ...
            # Plain paragraph
                ...
                html.append(f"<p style='margin:4px 0;'>{t}</p>")
    ...
    def _build_master_html(self, contact_summaries: Dict[str, str], run_date: str) -> str:
        """
        Build the full HTML email body — one clearly separated section per contact.
        """
        display_date = run_date.replace("_", " ")
        parts = [
    ...
    def send_summary_email(self, contact_summaries: Dict[str, str], run_date: str):
        ...
        html_body    = self._build_master_html(contact_summaries, run_date)
    ...
    def run(self, days_back: int = 7):
        ...
        try:
            ...
            master_md = f"# Master Weekly Summary — {run_date}\n\n"
            master_md += "\n\n---\n\n".join(
                f"## Contact: {sender}\n\n{summary}"
            ...
            self.send_summary_email(contact_summaries, run_date)
