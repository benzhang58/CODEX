...
class EmailSummarizer:
    ...
    _PRIORITY_FOLDERS = ["[Gmail]/All Mail", "All Mail", "INBOX", "Sent Messages", "[Gmail]/Sent Mail"]
    ...
    def _list_all_folders(self, mail: imaplib.IMAP4_SSL) -> List[str]:
        ...
        for f in folders:
            ...
            if name and name != '/':
                names.append(name)
        # Priority folders first, skip junk/trash/drafts entirely
        priority = [n for n in self._PRIORITY_FOLDERS if n in names]
        rest     = [n for n in names if n not in self._PRIORITY_FOLDERS and n not in self._SKIP_FOLDERS]
        ordered  = priority + rest
        logger.info(f"Folders discovered: {names}")
        logger.info(f"Thread search order: {ordered}")
        return ordered
        ...
        if clean_subject:
            ...
            for folder in subject_folders:
                ...
                try:
                    ...
                    uids = data[0].split() if data and data[0] else []
                    for uid_bytes in uids:
                        subject_hits.append((folder, int(uid_bytes)))
                except Exception:
                    continue
            self._restore_primary(mail)
    ...
    def generate_summary(self, records: List[EmailRecord], contact_name: Optional[str] = None, is_overall: bool = False) -> str:
        ...
        if all_attachment_names:
            attachment_instruction = (
                ...
                + "\n\nFor each bullet: state the filename, who sent it, what it contains, "
                "and why it matters. No dates.\n\n"
            )
        else:
            attachment_instruction = ""
        ...
        # Debug: log how many unique attachments made it into the prompt
        for record in records:
            latest_by_filename: Dict[str, AttachmentInfo] = {}
            for tm in record.thread:
                for att in tm.attachments:
                    latest_by_filename[att.filename] = att
            for fname, att in latest_by_filename.items():
                logger.info(f"  ATTACHMENT IN PROMPT: {fname} | preview_len={len(att.preview)} | sender={att.from_sender}")
